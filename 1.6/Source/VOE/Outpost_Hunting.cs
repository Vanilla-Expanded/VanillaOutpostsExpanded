...
public class Outpost_Hunting : Outpost_ChooseResult
{
            ...
            return new List<ResultOption>
            {
                new()
                {
                    ...
                    AmountsPerSkills = new List<AmountBySkill>
                    {
                        new()
                        {
                            ...
                            Count = (int)(ProductionMultiplier * Shooting * Leather * opt.Thing.GetStatValueAbstract(StatDefOf.LeatherAmount)
                                        * opt.AmountsPerSkills[0].Count)
                        },
                        new()
                        {
                            Skill = SkillDefOf.Animals,
                            Count = (int)(ProductionMultiplier * Animals * Leather * opt.Thing.GetStatValueAbstract(StatDefOf.LeatherAmount)
                                        * opt.AmountsPerSkills[0].Count)
                        }
                    }
                },
                new()
                {
                    ...
                    AmountsPerSkills = new List<AmountBySkill>
                    {
                        ...
                        {
                            Skill = SkillDefOf.Shooting,
                            Count = (int)(ProductionMultiplier * Shooting * Meat * opt.Thing.GetStatValueAbstract(StatDefOf.MeatAmount)
                                        * opt.AmountsPerSkills[0].Count)
                        },
                        new()
                        {
                            Skill = SkillDefOf.Animals,
                            Count = (int)(ProductionMultiplier * Animals * Meat * opt.Thing.GetStatValueAbstract(StatDefOf.MeatAmount)
                                        * opt.AmountsPerSkills[0].Count)
                        }
                    }
                }
    ...
    {
        var biome = Find.WorldGrid[Tile].PrimaryBiome;
        return biome.AllWildAnimals.Where(pkd => pkd.RaceProps.baseBodySize >= 2f)
           .OrderByDescending(biome.CommonalityOfAnimal)
           .Take(5)
